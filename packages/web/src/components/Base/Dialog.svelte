<script>
  ...
  import { ClassBuilder } from "commloc/tailwind/classes.js";

  const classesDefault = "items-center z-50 rounded bg-white p-4 shadow";
...
{#if opened}
  <div class="fixed w-full h-full top-0 left-0 z-30">
    <Scrim {opacity} on:click={() => !persistent && (opened = false)} />
  ...
  {:else}
  <h1 class="mt-16">Opened is false</h1>
{/if}
