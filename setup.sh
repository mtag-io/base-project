...
if [ ! -h .env ];
  then
    ln -s "$UPPER"/@env-pool/base-project/.env .
fi

if [ ! -h "$ROOT"/packages/server/.env ];
  then
    ln -s "$UPPER"/@env-pool/base-project/.env "$ROOT"/packages/server/.env
fi
...
if [ ! -h "$ROOT"/@common ];
  then
    ln -s "$ROOT"/packages/common/ "$ROOT"/@common
fi
...
if [ ! -h ./packages/web/@common ];
  then
    ln -s ./packages/common/ .packages/web/@common
fi
